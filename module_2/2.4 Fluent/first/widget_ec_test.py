...
class CoursesNavigationWidget:
    ...
    def is_loaded(self):
        try:
            self._get_buttons()  # we can use it here too
            # TODO add new check about visibility
            return WebDriverWait(self.driver, 10).until(
                EC.visibility_of_any_elements_located(self.BUTTONS)
            )
...
class CoursePage(object):
    ...
    SEARCH_INPUT = (
        By.XPATH,
        '//input[@placeholder="Профессия, навык"]',
    )
    CHAT_BUTTON = (By.XPATH, '//div[@class="sb-textback"]')
    ...
    def is_opened(self):
        ...
        try:
            WebDriverWait(self.driver, 10).until(
                lambda driver: EC.visibility_of_element_located(self.SEARCH_INPUT)(
                    driver
                )
                or EC.element_to_be_clickable(self.CHAT_BUTTON)(driver)
            )
        except WebDriverException:
            return False
...
def test_with_ec(driver, about: str, is_courses: bool, is_professions: bool):
    # arrange
